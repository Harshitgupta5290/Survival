...
func _handle_combat() -> void:
	...
	if Input.is_action_just_pressed("switch_weapon"):
		WeaponManager.cycle_weapon()
		current_weapon = WeaponManager.get_name()
		emit_signal("weapon_changed", current_weapon)
		# Reload ammo for the new weapon
		var ws := WeaponManager.get_stats()
		ammo = ws["ammo"]
		emit_signal("ammo_changed", ammo)

	var ws := WeaponManager.get_stats()
	var cooldown : float = ws["cooldown"]

	if InputManager.is_shooting() and shoot_timer <= 0.0 and ammo > 0:
		_shoot()
		shoot_timer = cooldown
	elif InputManager.is_shooting() and shoot_timer <= 0.0 and ammo == 0:
		shoot_timer = 0.1   # brief lock to avoid rapid empty-click sound
...
func _shoot() -> void:
	...
	var spread   : float = ws["spread"]
	var damage   : int   = ws["damage"]
...
func take_damage(amount: int) -> void:
	...
	emit_signal("damage_taken", amount, global_position)
	if health <= 0:
		health = 0
		_die()
